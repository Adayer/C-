...
struct Exercise3 {
	...
	static vec3 getWorldMousePosition(float mouse_x, float mouse_y, float windowsWidth, float windowsHeight, const mat4& projMat, const mat4& viewMat) {
		...
		vec4 mousePos(mouse_x, mouse_y, -1, 1);

		//Multiply by the inverse of the projection matrix to get into camera space
		mousePos =  inverse(projMat)*mousePos;
		mousePos = vec4(mousePos.x, mousePos.y, mousePos.z, 1);

		//Multiply by the inverse of the view matrix to get into world space
		mousePos = inverse(viewMat) * mousePos;
	...
	static bool raySphereIntersection(const Ray& ray, vec3 C, float r, float* intersection_distance) {
		...
		assert(fabsf(length(D) - 1) < 1e-03);

		vec3 L = C - O; // L: vector from ray origin to the center of the sphere
		float tca = dot(L, D);  // tca: L proyection on the ray (distance between origin and the point of the ray perpendicular to the center of the sphere)

		// If tca is negative, the sphere is in the oposite direction of the ray, behind the origin, so there is no intersection unless the origin is inside the sphere

		float d2 = dot(L, L) - tca * tca; // d:	shortest distance between the ray and the center, if it's larger than the radius, there's no intersection
		if (d2 > r*r)
		{
			return false;
		}
		...
		if (t0<0)
		{
			if (t1 < 0)
			{
				return false; // t0 & t1 negative: both intersections are in the oposite direction
			}
			else
			{
				*intersection_distance = t1; // t0 negative & t1 positive: the origin is inside the sphere, so there's just one intersection
			}
		}
		else if (t1<0)
		{
			*intersection_distance = t0; // t1 negative & t0 positive: the origin is inside the sphere, so there's just one intersection
		}
		else
		{
			*intersection_distance = (t0 < t1) ? t0 : t1; // t0 & t1 positive: two intersections, returns the closest to the origin
			// If t1=t0 the ray is tangent to the sphere
		}
		return true;
	}
