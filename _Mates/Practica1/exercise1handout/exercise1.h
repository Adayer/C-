...
struct Exercise1 {
	...
	void update() {
		...
		if (glfwGetKey(window, GLFW_KEY_UP)) {
			cameraPosition = vec3(cameraPosition.x, cameraPosition.y + 1 * elapsed_seconds, cameraPosition.z);
		}
		if (glfwGetKey(window, GLFW_KEY_DOWN)) {
			cameraPosition = vec3(cameraPosition.x, cameraPosition.y - 1 * elapsed_seconds, cameraPosition.z);
		}
		if (glfwGetKey(window, GLFW_KEY_RIGHT)) {
			cameraPosition = vec3(cameraPosition.x + 1 * elapsed_seconds, cameraPosition.y , cameraPosition.z);
		}
		...
		if (glfwGetKey(window, GLFW_KEY_W)) {
			cubeMatrix = translate(cubeMatrix, vec3(cubePosition.x, cubePosition.y + 1 * elapsed_seconds, cubePosition.y));
		}
		if (glfwGetKey(window, GLFW_KEY_S)) {
			cubeMatrix = translate(cubeMatrix, vec3(cubePosition.x , cubePosition.y - 1 * elapsed_seconds, cubePosition.y));
		}
		if (glfwGetKey(window, GLFW_KEY_D)) {
			cubeMatrix = translate(cubeMatrix, vec3(cubePosition.x + 1 * elapsed_seconds, cubePosition.y , cubePosition.y));
		}
		...
		if (glfwGetKey(window, GLFW_KEY_Q)) {
			cubeMatrix = rotate_z_deg(cubeMatrix, 50 * elapsed_seconds);
		}
