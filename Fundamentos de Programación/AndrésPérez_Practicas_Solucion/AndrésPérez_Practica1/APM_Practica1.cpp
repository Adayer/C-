...
int main() {
	...
	unsigned int uDatosJugadorAlterado(0x054D400C);

	//Bits del 23-16 son las balas
	std::cout << GetNumBullets(uDatosJugador) << "\n";
	//Añadir balas 
	uDatosJugadorAlterado = AddBullets(uDatosJugador, 10);
	std::cout << GetNumBullets(uDatosJugadorAlterado) << "\n";
	//check if bullet time is active
	std::cout << GetIsBulletTimeActive(uDatosJugadorAlterado) << "\n";
	uDatosJugadorAlterado = ToogleBulletTime(uDatosJugadorAlterado);
	std::cout << GetIsBulletTimeActive(uDatosJugadorAlterado) << "\n";
	return 0;
}
